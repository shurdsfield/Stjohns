...
def default_season():
    """Suggest the season covering the next round of fixtures.

    A season runs Aug → May. From Jun onwards we're into the next season's
    pre-season friendlies, so jump ahead. Returns ('2026-27', 2026)."""
    today = datetime.date.today()
    # June or later = the new season starts this August
...
def parse_season_start_year(season):
    """Pull the start year out of '2026-27' / '2026–27' / '2026/27'."""
    m = re.match(r"\s*(\d{4})\b", season)
...
def main():
    ...
    suggested_season, _ = default_season()
    print()
    print(f"  Season runs Aug → May (friendlies start in pre-season).")
    print(f"  Use the start year + 2-digit end year, e.g. '{suggested_season}'.")
    season      = ask("Season", default=suggested_season)
    season_year = parse_season_start_year(season)
    if season_year is None:
        print(f"  WARNING: couldn't parse a start year from '{season}'.")
    ...
    if has_chat:
        ...
        # Suggest 1 Aug of the season's start year as the cutoff
        if season_year is not None:
            hint_default = f"8/1/{str(season_year)[2:]}"
            print()
            print(f"  This date is used to skip chat messages from prior seasons.")
            print(f"  Default {hint_default} (1 Aug {season_year}) captures pre-season friendlies.")
        else:
            hint_default = ""
        start_hint = ask(
            "First-match date hint (M/D/YY)",
    ...
    if season_year is not None:
        # Aug 1 start year → May 31 end year — the rough window the parser
        # should consider in scope for this season.
        team_block["season_start"] = f"{season_year}-08-01"
        team_block["season_end"]   = f"{season_year + 1}-05-31"
